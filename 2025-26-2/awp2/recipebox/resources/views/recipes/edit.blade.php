...
@section('content')
  ...
  <main class="px-4 lg:px-8 py-8 max-w-3xl mx-auto w-full flex-1">
    ...
    <form action="{{ route('recipes.update', ['recipe' => $recipe->id]) }}" enctype="multipart/form-data" method="post" class="space-y-6">
      @csrf
      @method('put')
      <!-- Title -->
      ...
      <fieldset class="fieldset">
        ...
        <div class="flex flex-wrap gap-3">
          ...
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="categories[]" value="{{ $category->id }}" class="checkbox checkbox-sm checkbox-primary" {{ in_array($category->id, old('categories', $recipe->categories->pluck('id')->toArray())) ? 'checked' : '' }}  >
